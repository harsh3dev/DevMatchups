            ...
            <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg  md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0  dark:border-gray-700">
              {navItems.map((item) => (
                <li key={item.name}>
                  <Link
                    href={item.path}
                    className={`block py-2 px-3 text-text rounded text-sm  md:hover:text-accent md:p-0 md:dark:hover:text-accent dark:text-white  `}
                    aria-current="page"
                    style={{
                      color: pathname === item.path ? "var(--accent)" : "",
                    }}
                    key={`list-${item.name}`}
                  >
                    {item.name}
          ...
          <div className=" hidden md:flex justify-between gap-2 md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
            {status === "unauthenticated" && (
              <div className="flex justify-center items-center gap-2">
                ...
                <Button
                  variant={"ghost"}
                  className=" group dark:hover:bg-accent hover:bg-accent hover:text-white "
                >
                  <Link
                    href={"/login"}
                    className="text-accent rounded-lg border border-accent p-2 lg:text-text lg:border-0 group-hover:text-white "
                  >
                    Login
                  </Link>
                </Button>
              ...
              <SheetContent side="right" className="w-[300px] bg-background">
                  ...
                  <>
                    <div className=" w-full flex flex-col justify-center items-center gap-2 mb-2 ">
                      <Link
                        ...
                        className="w-full flex justify-start items-center gap-4 border border-gray-600 dark:border-gray-800 rounded-md p-4 "
                      >
                        <span>
                          {" "}
                          <RiAccountBoxFill />{" "}
                      ...
                      <Link
                        href={`/dashboard`}
                        className="w-full flex justify-start items-center border gap-4 border-gray-600 dark:border-gray-800 rounded-md p-4 "
                      >
                        <span>
                          {" "}
                          <MdDashboard />{" "}
                ...
                {status === "unauthenticated" && (
                  <div className=" flex flex-col gap-4 ">
                    <div className=" flex  md:hidden justify-center gap-2 md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
                      ...
                      <Button
                        variant={"ghost"}
                        className="w-full outline outline-1 outline-accent"
                      >
                  ...
                  {navLinks.map((item, index) => (
                    <React.Fragment key={`index-${index}`}>
                      ...
                      >
                        {item.icon}
                        <span
                          className="text-text dark:text-text transition-all ease-linear"
                          style={{
                            color:
                ...
                <div className=" w-full flex flex-col gap-2 items-center justify-center">
                  <div className="w-full flex items-center justify-center gap-2">
                    ...
                    <Link href="mailto:harshpandey.tech@gmail.com">
                      <Button
                        variant="outline"
                        className="flex justify-center items-center gap-2 w-full"
                      >
